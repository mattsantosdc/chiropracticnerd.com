...
Every change to statements, structured arguments, their schemas or page templates, their reading-path configuration and derived reasoning data, or their governing policy requires an updated semantic audit. The working AI agent or a human editor performs the audit; a local script checks that the recorded review covers the current files. This is a focused whole-Model check for v0.1, separate from the deferred incremental engine in [review-invalidation-plan.md](review-invalidation-plan.md).
...
**Adoption is separate from evidential confidence.** Inclusion in a Model version identifies a statement or argument as part of its current working account. Unresolved support does not make an included claim unadopted, and adoption does not establish truth or inferential validity. Review this distinction in metadata, prose, and reader-facing presentation. Statement and argument records have no editorial `status` property or status badges.

## Rubric for the reviewer

For every statement and argument, record specific findings under these five fields:
...
3. Perform the rubric above across the whole Model and all structured arguments. At v0.1's size this intentionally includes unchanged statements, so revisions cannot escape review through a missing relationship. Reconsider existing findings; retain a finding only after checking that it still applies.
...
5. Only after reviewing the final inputs, update `reviews/model-review.json`. Record `schemaVersion: 1`, reviewer provenance, an ISO `reviewedAt` timestamp, the packet's `inputs` map, and a `records` object keyed by every statement and argument file path. Each record has `finding` and all five nonempty rubric fields. AI reviewers identify the agent and model, stating when the exact runtime revision is unavailable.
6. Run `npm test`, `npm run build`, and `npm run test:routes`. The test suite and build require a current completed review; route tests inspect the built output. Any later content or policy change requires renewed review before recording new fingerprints.
...
The schema-1 fingerprint is SHA-256 of each file's UTF-8 text after normalizing CRLF to LF. Paths are repository-relative and sorted. All statement and argument `.md` and `.mdx` files are collected from the two exact canonical directories, `src/content/model/statements/` and `src/content/model/arguments/`, exactly once, along with `AGENTS.md`, the five authoring contracts, this policy, the audit script, `src/content.config.ts`, the four overview/detail page templates for statements and arguments, and their shared presentation inputs: `src/layouts/Layout.astro`, `src/lib/site.ts`, `src/components/ModelAcknowledgment.astro`, `src/components/DependencyLegend.astro`, and `src/components/Comments.astro`. The statement, argument, identifier, and dependency helpers in `src/lib/` are covered as well. Reading inputs are explicitly included: `src/data/model-reading-path.json`, `docs/model-reading-path.md`, `src/lib/reading-path.ts`, and `src/lib/reasoning.ts`. Add any new production input capable of changing the presented account to `policyPaths`; discovery is not automatic. Keep one review record per canonical statement or argument, recording placement, section prose, and derived-relationship findings in the existing rubric fields. The Stage 4 integration note documents the overall path review; it and generated inspection output are explanatory bookkeeping, not production inputs. Files added, removed, renamed, or edited invalidate the recorded input set. Policy, checker, schema, or presentation changes also invalidate it, so a presentation-only change cannot reintroduce a misleading adoption label without renewed review. The complete input set is included in the packet, and stale reports name the changed paths.
...
The review JSON is excluded from the input set, so recording findings cannot invalidate its own inputs. It must contain exactly one complete record per current statement or argument file. Tests check freshness, coverage, provenance, record shape, and recorded findings; they cannot determine whether the reviewer reasoned correctly. A fresh, structurally complete but careless review can still be wrong. There is no automatic empirical appraisal, proof of validity, or guarantee that an AI catches every problem.
