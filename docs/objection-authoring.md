...
During static builds and development, the shared reader lookup checks the actual alternative Markdown directories before querying Astro. An intentionally empty directory returns an empty list without Astro's missing-or-empty warning. Missing directories still fail, populated directories must load entries, and the full binding/inventory validation always runs. Never suppress global warnings or infer an empty corpus from empty bindings; either could hide a real authoring problem.
