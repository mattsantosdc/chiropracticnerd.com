## ASPIC+ foundation and branch workflow

The Model is a network of statements with question-specific conclusions, not one global beginning and ending. Follow [the ASPIC+ foundation contract](docs/aspic-foundation.md), [objection authoring](docs/objection-authoring.md), and [the relationship contract](docs/dependency-model.md) for the formal foundation and canonical relationships.

Create follow-up branches from current `main`; eventual pull requests target `main`, unless the user specifies another branch or base. `reasoning/dependency-migration.json` preserves the relationship migration inventory and its limiting notes; `docs/model-opposition-audit.md` records the substantive audit and remaining empirical and normative questions. The user will review and merge manually; do not merge into `model-v0.1` or `main`. Continue implementation and authorized branch commits without interrupting the work for PR preparation. Keep governing instructions current with each schema, engine, content and presentation change. Use [the foundation completion checklist](docs/foundation-status.md) to distinguish implemented capabilities from pending interoperability and deferred standards.
...
Keep unrelated navigation redesigns separate from foundation changes. Reader context necessary to expose newly admitted opposition belongs with the corresponding schema change.
...
Author alternative propositions in `src/content/model/alternatives/` and their inference applications in `src/content/model/alternative-arguments/`, using shared schemas and globally unique S/ARG identities. Bind them exactly in `reasoning/opposition-bindings.json`. Explicit premise admissions and undercut designations require rationales. A signed reference such as `-S-011` means the exact classical negation of the complete proposition, not a weaker observation or different explanation. All recorded applications join the complete theory; no argument is generated without admitted or derived premises. Collection role never grants priority, starting-premise membership or acceptance. Hypothetical experiments remain in the isolated scenario or test layer. Do not manufacture substantive counterclaims to populate an empty collection. Include new records, bindings, negative endpoints and attacks in semantic review, full evaluation and reader context.
...
Review globally when shared governing meaning, schemas, reasoning implementation/profile/signature or shared presentation changes, when an unsupported relationship cannot be mapped, before a Model-version release, and at least every 90 days. Editorial documentation changes alone do not require a whole-model review: inspect the exact diff and follow [the documentation-review workflow](docs/model-review.md#editorial-documentation-changes). For registered documents, record an explicit `editorial-only` attestation with exact before/after fingerprints and reasons through `--record-docs-review`; preserve existing findings, their reviewer/timestamps and the whole-model date. Do not classify an actual rule or meaning change as editorial. Use `--full` to request a whole-model review plan. The periodic gate is checked during ordinary project commands, not by a scheduled task. Inspect for missing relationships during whole-model reviews. Never refresh outer snapshot fingerprints or per-record bases merely to clear a gate. Record findings only after examining the applicable content. A derived impact set can be conservative and cannot discover an unrecorded semantic relationship by itself.

`npm test` and `npm run build` require current review coverage. Run the complete reasoning suite and whole-theory pilot after every semantic or implementation Model change; incremental semantic review does not authorize sliced evaluation or skipping automated integrity checks. Editorial documentation-only changes need link/anchor checks, `git diff --check`, and the review check for registered inputs; they do not require rerunning reasoning, builds or browser checks. Empirical confidence may remain unresolved. No separate human approval is required for review bookkeeping or implementation. Incremental formal evaluation and a hosted AI reviewer remain deferred.
