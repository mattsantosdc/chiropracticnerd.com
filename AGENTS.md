...
The user has merged the foundation, relationship migration and navigation into `main`. Create follow-up branches from current `main`; eventual pull requests target `main`, unless the user specifies another base. The relationship decisions and canonical relationship migration are complete. `docs/aspic-migration.md` records their dispositions; `docs/model-opposition-audit.md` records the substantive audit and remaining empirical and normative questions. The user will review and merge manually; do not merge into `model-v0.1` or `main`. Continue implementation and branch commits without interrupting the work for PR preparation. Keep governing instructions current with each schema, engine, content and presentation change. Use [the foundation completion checklist](docs/foundation-status.md) to distinguish implemented capabilities from pending interoperability and deferred standards.
...
The navigation work was reviewed separately and is now merged. Keep future unrelated navigation redesigns separate from foundation changes. Reader context necessary to expose newly admitted opposition belongs with the corresponding schema change.
