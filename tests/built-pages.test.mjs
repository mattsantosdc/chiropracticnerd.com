...
test('all walkthrough appearances and supporting material match the canonical records', async () => {
	...
	for (const appearance of nodes(overview).filter((node) => attr(node, 'data-statement-id'))) {
		...
		assert.ok(metadata.includes(entry.data.confidence === 'not-applicable' ? 'Not applicable' : entry.data.confidence));
	}
	for (const entry of [...statements, ...argumentsList]) {
		...
		assert.ok(bodies.length, `${entry.data.id} body missing`);
		...
		assert.ok(normalize(text(overview)).includes(normalize(entry.data.summary)), `${entry.data.id} summary`);
		if (entry.data.whatWouldChange) assert.ok(normalize(text(overview)).includes(normalize(entry.data.whatWouldChange)), `${entry.data.id} revision condition`);
		for (const ref of entry.data.references ?? []) {
			assert.ok(links(overview).includes(ref.url), `${entry.data.id} reference`);
			if (ref.note) assert.ok(normalize(text(overview)).includes(normalize(ref.note)), `${entry.data.id} source limit`);
		}
	}
});

test('all participation remains visible near statements and distinct from revision dependencies', () => {
	const overview = readPage('/model/');
	for (const entry of statements) {
		for (const page of [overview, readPage(statementHref(entry))]) {
	...
	const s7 = nodes(overview).find((node) => attr(node, 'data-participation') === 'S-007');
